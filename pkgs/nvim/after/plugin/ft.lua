...
------------- Markdown ---------
--------------------------------
-- .mdx has neither builtin filetype detection (the content heuristic guesses
-- `conf`) nor a treesitter grammar, so pin the extension and point it at the
-- markdown parser. Consumers that enumerate filetypes -- render-markdown,
-- markdown-preview, the zk LSP, mini.snippets -- list "mdx" explicitly.
