...
function taskFor(request: string): string {
	return [
		...
		"",
		"If the request needs particular content on screen — a game played, a feature demoed — check",
		"FIRST whether this project can stage it for you: run `harness-skill-pick \"<the request>\"` and",
		"look in `.agents/skills/` and `scripts/`. A project that owns its own recorder (the games repo",
		"has `scripts/make-preview.sh`, via its `game-previews` skill) is the correct path — use it",
		"instead of screen capture, and follow that skill over anything here.",
		"Only if nothing in the project stages the content: you capture what is already on screen and",
		"cannot play the user's games or drive their apps. Do NOT record an idle desktop and do NOT keep",
		"trying — stop immediately and report that the user has to perform it, with the exact commands",
