...
vim.keymap.set("v", "<leader>ts", ":FloatermSend<cr>", { desc = "send selection to term" })
...
end, { desc = "Open file under cursor (or file within environment under cursor)." })
vim.keymap.set("n", "<leader>if", function()
	illustrate_finder.search_and_open()
end, { desc = "Use telescope to search and open illustrations in default app." })
vim.keymap.set("n", "<leader>ic", function()
	illustrate_finder.search_create_copy_and_open()
end, { desc = "Use telescope to search existing file, copy it with new name, and open it in default app." })

-- External Commands
wk.add({ "<leader>e", desc = "External commands" })
vim.keymap.set("n", "<leader>ex", ":.w !bash -e<cr>", { desc = "run bash on current line" })
vim.keymap.set("n", "<leader>eX", ":%w !bash -e<cr>", { desc = "run bash on all lines" })
vim.keymap.set("n", "<leader>el", ":.!bash -e<cr>", { desc = "replace bash output on current line" })
vim.keymap.set("n", "<leader>eL", ":% !bash -e<cr>", { desc = "replace bash output on all lines" })
vim.keymap.set("n", "<leader>ef", ":lua *G.execute*file_and_show_output()<cr>", { desc = "run current file" })
vim.keymap.set("n", "<leader>cx", ":!chmod +x %<cr>", { desc = "make current file executable" })
